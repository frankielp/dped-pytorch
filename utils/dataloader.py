...
class DPEDTestDataset(Dataset):
    ...
    def load(self, datadir, test_subset):
        test_image = sorted([os.path.join(datadir, name) for name in os.listdir(datadir)
        ...
        IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_SIZE =get_specified_res(self.phone, self.resolution)
...
class DPEDTrainDataset(Dataset):
    def __init__(self, phone, datadir,train_size=-1,is_train=True): # train_size=-1 (full)
        super().__init__()
        self.train_image=[]
        self.phone=phone
    ...
    def load(self,datadir):
        if self.is_train:
            phone_dir = datadir + str(self.phone) + '/training_data/' + str(self.phone) + '/'
            dslr_dir = datadir + str(self.phone) + '/training_data/canon/'
            
            NUM_TRAINING_IMAGES = len([name for name in os.listdir(phone_dir)
            ...
            if self.train_size == -1:
                ...
                train_image = np.random.choice(np.arange(0, NUM_TRAINING_IMAGES), self.train_size, replace=False)
            ...
            for img_path in tqdm(train_image):
                ...
                phone_image=np.float16(np.array(Image.open(phone_image_path))) / 255  # 0-255 -> 0-1
                phone_image_2d=np.reshape(phone_image, [1, PATCH_SIZE])
                phone_image_tensor=self.transform(phone_image_2d)

                dslr_image=np.float16(np.array(Image.open(dslr_image_path))) / 255  # 0-255 -> 0-1
                dslr_image_2d=np.reshape(dslr_image, [1, PATCH_SIZE])
                dslr_image_tensor=self.transform(dslr_image_2d)

                self.train_image.append({'phone_image':phone_image_tensor,'dslr_image':dslr_image_tensor})
            
        else:
            phone_dir = datadir + str(self.phone) + '/test_data/patches/' + str(self.phone) + '/'
            dslr_dir = datadir + str(self.phone) + '/test_data/patches/canon/'

            NUM_TEST_IMAGES = len([name for name in os.listdir(phone_dir)
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,collate_fn=train_dataset.collate_fn)
